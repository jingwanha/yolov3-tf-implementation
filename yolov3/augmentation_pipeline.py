...
def basic_augmentation(image,boxes):
    image_np = image.astype(np.uint8)
    width, height, _ = image_np.shape   
    
    bbs = []
    for box in boxes:        
        xmin,ymin,xmax,ymax,label = box[0],box[1],box[2],box[3],box[4]
        bbs.append(BoundingBox(x1=xmin*width, y1=ymin*height,x2=xmax*width, y2=ymax*height,label=label))

    bbs = BoundingBoxesOnImage(bbs, shape=image_np.shape)
    image_aug, bbs_aug = aug_pipeline(image=image_np, bounding_boxes=bbs) 
    ...
    labels_aug = []
    for bb in bbs_aug:
        boxes_aug.append([bb.x1/width, bb.y1/height,  bb.x2/width, bb.y2/height, bb.label])

    boxes_aug = np.array(boxes_aug)
    if len(boxes_aug) == 0: boxes_aug=np.array([[0.,0.,0.,0.,0.]])
        
    labels_aug = np.array(labels_aug)

    merge = np.concatenate((image_aug, boxes_aug), axis=None)
    return tf.constant(image_aug,dtype='float32'), tf.constant(boxes_aug,dtype='float32')
